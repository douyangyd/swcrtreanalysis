...
fit.data.poisson.CO <- function(
  ...
  if(is.null(offset)){
    ...
    ## Immediate time model (IT)
    ### Model fitting
    model1 <- try(glmer(Outcome ~  Period + Treatment + (1|Cluster) + (1|Cluster:Period) + (1|id_individual) + offset(log(off)), 
                        family = "poisson", data = data), silent = TRUE)
    
    # Check if model1 converged
    if (check_convergence(model1)[1] == F) {
      # Fit the next model if model1 failed to converge
      model2 <- try(glmer(Outcome ~  Period + Treatment + (1|Cluster) + (1|Cluster:Period) + offset(log(off)), 
                          family = "poisson", data = data), silent = TRUE)
      
      # Check if model2 also failed to converge
      if (check_convergence(model2)[1] == F) {
        model3 <- try(glmer(Outcome ~  Period + Treatment + (1|Cluster) + offset(log(off)), 
                            family = "poisson", data = data), silent = TRUE)
        if(check_convergence(model3)[1] == F) {
          ITconverge <- 0
          message("IT All models failed to converge.")
    ...
    ## Exposure time indicator model (ETI)
    ### Model fitting
    model1 <- try(glmer(Outcome ~  Period + Exposure + (1|Cluster) + (1|Cluster:Period) + (1|id_individual) + offset(log(off)), 
                        family = "poisson", data = data), silent = TRUE)
    
    # Check if model1 converged
    if (check_convergence(model1)[1] == F) {
      # Fit the next model if model1 failed to converge
      model2 <- try(glmer(Outcome ~  Period + Exposure + (1|Cluster) + (1|Cluster:Period) + offset(log(off)), 
                          family = "poisson", data = data), silent = TRUE)
      
      # Check if model2 also failed to converge
      if (check_convergence(model2)[1] == F) {
        model3 <- try(glmer(Outcome ~  Period + Exposure + (1|Cluster) + offset(log(off)), 
                            family = "poisson", data = data), silent = TRUE)
        if(check_convergence(model3)[1] == F) {
          ETIconverge <- 0
          message("ETI All models failed to converge.")
    ...
    ## Treatment effect hetero (TEH)
    # Fit the first model
    model1 <- try(glmer(Outcome ~  Period + Treatment + (0 + Treatment|Exposure) + (1|Cluster) + (1|Cluster:Period) + (1|id_individual) + offset(log(off)), 
                        family = "poisson", data = data), silent = TRUE)
    
    # Check if model1 converged
    if (check_convergence(model1)[1] == F) {
      # Fit the next model if model1 failed to converge
      model2 <- try(glmer(Outcome ~  Period + Treatment + (0 + Treatment|Exposurer) + (1|Cluster) + (1|Cluster:Period) + offset(log(off)), 
                          family = "poisson", data = data), silent = TRUE)
      
      # Check if model2 also failed to converge
      if (check_convergence(model2)[1] == F) {
        model3 <- try(glmer(Outcome ~  Period + Treatment + (0 + Treatment|Exposure) + (1|Cluster) + offset(log(off)), 
                            family = "poisson", data = data), silent = TRUE)
        if(check_convergence(model3)[1] == F) {
          TEHconverge <- 0
          message("TEH All models failed to converge.")
    ...
    b_vars <- paste0("b", 1:(nnode))
    formula1 <- as.formula(paste("Outcome ~ factor(Period) + ", paste(b_vars, collapse = " + "), "+ (1|Cluster) + (1|Cluster:Period) + (1|id_individual) + offset(log(off)) "))
    formula2 <- as.formula(paste("Outcome ~ factor(Period) + ", paste(b_vars, collapse = " + "), "+ (1|Cluster) + (1|Cluster:Period) + offset(log(off))"))
    formula3 <- as.formula(paste("Outcome ~ factor(Period) + ", paste(b_vars, collapse = " + "), "+ (1|Cluster) + offset(log(off))"))
