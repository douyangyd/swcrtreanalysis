...
fit.data.poisson.CS <- function(
    ...
    offset = NULL         # Name of offset column, if no offset term enter NULL
){
  results <- list()
  if (is.null(offset)){
    ## Immediate time model (IT)
    ### Model fitting
    model1 <- try(glmer(Outcome ~  Period + Treatment + (1|Cluster) + (1|Cluster:Period), 
    ...
    if (check_convergence(model1)[1] == F) {
      ...
      model2 <- try(glmer(Outcome ~  Period + Treatment + (1|Cluster), 
                          family = "poisson", data = data), silent = TRUE)
      
      # Check if model2 also failed to converge
      if (check_convergence(model2)[1] == F) {
        ITconverge <- 0
        message("IT Both models failed to converge.")
      } else {
        IT <- model2
        ITconverge <- 2
    ...
    ### Model fitting
    model1 <- try(glmer(Outcome ~  Period + Exposure + (1|Cluster) + (1|Cluster:Period), 
                        family = "poisson", data = data), silent = TRUE)
    
    # Check if model1 converged
    if (check_convergence(model1)[1] == F) {
      # Fit the next model if model1 failed to converge
      model2 <- try(glmer(Outcome ~  Period + Exposure + (1|Cluster), 
                          family = "poisson", data = data), silent = TRUE)
      
      # Check if model2 also failed to converge
      if (check_convergence(model2)[1] == F) {
        ETIconverge <- 0
        message("ETI Both models failed to converge.")
      } else {
        ETIconverge <- 2
        ETI <- model2
    ...
    # Fit the first model
    model1 <- try(glmer(Outcome ~  Period + Treatment + (0+Treatment|Exposure) + (1|Cluster) + (1|Cluster:Period), 
                        family = "poisson", data = data), silent = TRUE)
    if (check_convergence(model1)[1] == F) {
      # Fit the next model if model1 failed to converge
      model2 <- try(glmer(Outcome ~  Period + Treatment + (0+Treatment|Exposure) + (1|Cluster), 
                          family = "poisson", data = data), silent = TRUE)
      
      # Check if model2 also failed to converge
      if (check_convergence(model2)[1] == F) {
        TEHconverge <- 0
        message("TEH Both models failed to converge.")
      } else {
        TEH <- model2
        TEHconverge <- 2
    ...
    if (check_convergence(model1)[1] == F) {
      ...
      if (check_convergence(model2)[1] == F) {
        NCSconverge <- 0
        message("NCS Both models failed to converge.")
      } else {
        model <- model2
        NCSconverge <- 2
    ...
    ## Immediate time model (IT)
    ### Model fitting
    model1 <- try(glmer(Outcome ~  Period + Treatment + (1|Cluster) + (1|Cluster:Period) + offset(log(off)), 
                        family = "poisson", data = data), silent = TRUE)
    
    # Check if model1 converged
    if (check_convergence(model1)[1] == F) {
      # Fit the next model if model1 failed to converge
      model2 <- try(glmer(Outcome ~  Period + Treatment + (1|Cluster) + offset(log(off)), 
                          family = "poisson", data = data), silent = TRUE)
    ...
    ## Exposure time indicator model (ETI)
    ### Model fitting
    model1 <- try(glmer(Outcome ~  Period + Exposure + (1|Cluster) + (1|Cluster:Period) + offset(log(off)), 
                        family = "poisson", data = data), silent = TRUE)
    
    # Check if model1 converged
    if (check_convergence(model1)[1] == F) {
      # Fit the next model if model1 failed to converge
      model2 <- try(glmer(Outcome ~  Period + Exposure + (1|Cluster) + offset(log(off)), 
                          family = "poisson", data = data), silent = TRUE)
    ...
    ## Treatment effect hetero (TEH)
    # Fit the first model
    model1 <- try(glmer(Outcome ~  Period + Treatment + (0+Treatment|Exposure) + (1|Cluster) + (1|Cluster:Period) + offset(log(off)), 
                        family = "poisson", data = data), silent = TRUE)
    if (check_convergence(model1)[1] == F) {
      # Fit the next model if model1 failed to converge
      model2 <- try(glmer(Outcome ~  Period + Treatment + (0+Treatment|Exposure) + (1|Cluster) + offset(log(off)), 
                          family = "poisson", data = data), silent = TRUE)
    ...
    nnode <-  ceiling((J)/2)
    ns_basis <- ns(c(0:(J-1)), knots = (1:(nnode-1))*(J-1)/(nnode - 1))
    ...
    b_vars <- paste0("b", 1:(nnode))
    formula1 <- as.formula(paste("Outcome ~ factor(Period) + ", paste(b_vars, collapse = " + "), "+ (1|Cluster) + (1|Cluster:Period) + offset(log(off)) "))
    formula2 <- as.formula(paste("Outcome ~ factor(Period) + ", paste(b_vars, collapse = " + "), "+ (1|Cluster) + offset(log(off)) "))
