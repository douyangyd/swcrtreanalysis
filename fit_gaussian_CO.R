...
fit.data.gaussian.CO <- function(
  ...
  ### Model fitting
  model1 <- try(lmer(Outcome ~  Period + Treatment + (1|Cluster) + (1|Cluster:Period) + (1|id_individual), 
  ...
  if (check_convergence(model1)[1] == F) {
    ...
    model2 <- try(lmer(Outcome ~  Period + Treatment + (1|Cluster) + (1|Cluster:Period), 
                         data = data), silent = TRUE)
    
    # Check if model2 also failed to converge
    if (check_convergence(model2)[1] == F) {
      model3 <- try(lmer(Outcome ~  Period + Treatment + (1|Cluster), 
                           data = data), silent = TRUE)
  ...
  ### Model fitting
  model1 <- try(lmer(Outcome ~  Period + Exposure + (1|Cluster) + (1|Cluster:Period) + (1|id_individual), 
                       data = data), silent = TRUE)
  
  # Check if model1 converged
  if (check_convergence(model1)[1] == F) {
    # Fit the next model if model1 failed to converge
    model2 <- try(lmer(Outcome ~  Period + Exposure + (1|Cluster) + (1|Cluster:Period), 
                         data = data), silent = TRUE)
    
    # Check if model2 also failed to converge
    if (check_convergence(model2)[1] == F) {
      model3 <- try(lmer(Outcome ~  Period + Exposure + (1|Cluster), 
                           data = data), silent = TRUE)
      if(check_convergence(model3)[1] == F) {
        ETIconverge <- 0
  ...
  # Fit the first model
  model1 <- try(lmer(Outcome ~  Period + Treatment + (0 + Treatment|Exposure) + (1|Cluster) + (1|Cluster:Period) + (1|id_individual), 
                       data = data), silent = TRUE)
  
  # Check if model1 converged
  if (check_convergence(model1)[1] == F) {
    # Fit the next model if model1 failed to converge
    model2 <- try(lmer(Outcome ~  Period + Treatment + (0 + Treatment|Exposure) + (1|Cluster) + (1|Cluster:Period), 
                         data = data), silent = TRUE)
    
    # Check if model2 also failed to converge
    if (check_convergence(model2)[1] == F) {
      model3 <- try(lmer(Outcome ~  Period + Treatment + (0 + Treatment|Exposure) + (1|Cluster), 
                           data = data), silent = TRUE)
      if(check_convergence(model3)[1] == F) {
        TEHconverge <- 0
  ...
  #TEH <- lmer(Outcome ~  Period + Treatment + (Exposure|Cluster) + (1|Cluster:Period), 
  #              data = data)
  if(TEHconverge == 0){
  ...
  model1 <- try(lmer(formula1, 
                       data = data), silent = TRUE)
  
  # Check if model1 converged
  if (check_convergence(model1)[1] == F) {
    # Fit the next model if model1 failed to converge
    model2 <- try(lmer(formula2, 
                         data = data), silent = TRUE)
    
    # Check if model2 also failed to converge
    if (check_convergence(model2)[1] == F) {
      model3 <- try(lmer(formula3, 
                           data = data), silent = TRUE)
      if(check_convergence(model3)[1] == F) {
        NCSconverge <- 0
