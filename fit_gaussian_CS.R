# Analyze the Gaussian outcome with cross-sectional design
fit.data.gaussian.CS <- function(
  ...
  ### Model fitting
  model1 <- try(lmer(Outcome ~  Period + Treatment + (1|Cluster) + (1|Cluster:Period), 
  ...
  if (check_convergence(model1)[1] == F) {
    ...
    model2 <- try(lmer(Outcome ~  Period + Treatment + (1|Cluster), 
                         data = data), silent = TRUE)
  ...
  ### Model fitting
  model1 <- try(lmer(Outcome ~  Period + Exposure + (1|Cluster) + (1|Cluster:Period), 
                       data = data), silent = TRUE)
  
  # Check if model1 converged
  if (check_convergence(model1)[1] == F) {
    # Fit the next model if model1 failed to converge
    model2 <- try(lmer(Outcome ~  Period + Exposure + (1|Cluster), 
                         data = data), silent = TRUE)
  ...
  # Fit the first model
  model1 <- try(lmer(Outcome ~  Period + Treatment + (0+Treatment|Exposure) + (1|Cluster) + (1|Cluster:Period), 
                       data = data), silent = TRUE)
  if (check_convergence(model1)[1] == F) {
    # Fit the next model if model1 failed to converge
    model2 <- try(lmer(Outcome ~  Period + Treatment + (0+Treatment|Exposure) + (1|Cluster), 
                         data = data), silent = TRUE)
